...
Other events: `stage_completed`, `stage_cached`, `stage_waiting` (a lane is idle because the named `waiting_for` stages have not finished), `completed`, `failed`, `cancelled`. Stages run in two lanes: geometry (decode through geometry) and color (photos, masks, cameras, then candidates onward). Photos and masks start immediately beside the geometry lane; cameras needs registered poses; candidates needs geometry, cameras and masks. Two `stage_started` events can therefore be open at once. Terminal stage events carry wall time, OS child CPU seconds, average core equivalents and sampled peak process-tree RSS. Metal blend receipts additionally contain hardware command time, device name and buffer allocation. Detailed mask and geometry receipts live inside their stage folders. CPU seconds are not percentages: 8 CPU seconds over 2 seconds wall means 4 average core equivalents.
